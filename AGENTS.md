...
- Atari 800XL: `PRINTER` and `TEXT_PRINTER` open `P:`; `RS232` opens `R:`; `SHARED_DRIVE` opens `H6:MCP.TXT` by default for Altirra H: host-device text capture. The Atari800 launch adapter overrides `SHARED_DRIVE` to `H1:MCP.TXT`. Availability is checked with `TRAP` around an `OPEN` where practical.
...
The order is significant and all listed files form one compilation unit. The first file contains entry code, and generated BASIC preserves the configured module order. Later files may define functions, structs, enums, arrays, and struct values used by earlier files because semantic analysis collects top-level declarations across the combined program before analyzing executable statements; constant declarations themselves still evaluate in source/build order. Generated startup calls initialize per-module storage and globals before entry code runs without moving later module sections ahead of the entry module. All `DATA` statements are emitted in a final data section. Paths are resolved relative to the configuration JSON, not the current working directory. Keep JSON loading in `src/build-configuration.ts`; the compiler core should accept an internal `BuildConfiguration`/program representation and must not depend on JSON.
...
`scripts/launch-atari800.mjs` backs `npm run launch:atari800` and uses the `atari800xl.emulators.atari800` block of `scripts/tools.local.json`. It targets Atari800 7.x style command-line launches, defaults to `-xl -pal -basic -H1 {sharedDrive} -hreadwrite -run {artifact}`, and overrides generated `SHARED_DRIVE` output to `H1:MCP.TXT`. The all-target launcher starts both Atari emulators by default when both are configured, chooses Atari800 when it is the only configured Atari emulator, and accepts `--atari-emulator auto|altirra|atari800|all`.
