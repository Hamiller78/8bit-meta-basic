...
function allocateSpectrumArrayNames(names: readonly string[], map: Map<string, string>): void {
  ...
  for (const name of names) {
    ...
    const next = nextSpectrumNumericName(used, "numeric or string arrays");
...
function allocateSpectrumLoopNames(names: readonly string[], reservedSingleLetters: ReadonlySet<string>, map: Map<string, string>): void {
  ...
  for (const name of names) {
    ...
    const next = nextSpectrumNumericName(new Set([...used, ...reservedSingleLetters]), "active FOR loop variables");
    map.set(key, next);
    used.add(next);
  }
}

function nextSpectrumNumericName(used: ReadonlySet<string>, resource: string): string {
  ...
  throw new Error(`Spectrum target only supports 26 ${resource}.`);
}
